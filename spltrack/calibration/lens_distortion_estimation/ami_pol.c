...
/**
 * \fn long double ami_horner(long double *pol,int degree,long double x,long
 * double *fp) \brief function to evaluate a polynomial and its derivate using
 * Horner Algorithm \return polynomial evaluation \author Luis Alvarez
 */
...
long double ami_root_bisection(
  ...
  while ((b2 - a2) > (TOL * fabs(b2) + 1e-10) &&
         fabs(c - c0) > (TOL * fabs(c0) + 1e-10) && (iter++) < maxiter) {
...
/**
 * \fn int ami_polynomial_root(double *pol, int degree, double *root_r, double
 *root_i)
 * \brief function to estimate real polynomial roots. We use a recursive
 procedure computing the derivative polynomial roots. This function only works
 for relative low polynomial degree
...
int ami_polynomial_root(
  ...
  max = fabs(pol2[0]);
  for (k = 1; k < N; k++) {
    if (fabs(pol2[k]) > max) max = fabs(pol2[k]);
  }
