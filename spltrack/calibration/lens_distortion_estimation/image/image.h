...
class image {
  std::vector<T>
      image_ /** std vector to allocate image "channel 1 + channel 2 + ..." */;
  int width_ /** image width */;
  int height_ /** image height */;
  int nChannels_ /** number of image channels */;
  std::vector<int>
      roi_ /** ROI : Region Of Interest =(x0,x1,y0,y1,c0,c1) subwindow corners
              used (if defined) to operate only in a subwindow area */
      ;
...
#pragma omp parallel for shared(i_end, n0, n_s, sampled_img, width_s)
    for (int i = 1; i < i_end; i++) {
      ...
      for (int j = 1; j < j_end; j++) {
        ...
        sampled_img[m_s + j] =
            0.25 * image_[m] +
            0.125 * (image_[m + 1] + image_[m - 1] + image_[m + width_] +
                     image_[m - width_]) +
            0.0625 * (image_[m + width_ + 1] + image_[m + width_ - 1] +
                      image_[m - width_ + 1] + image_[m - width_ - 1]);
      }
    }
  }
  ...
  for (int n = 0; n < nChannels_; n++) {
    ...
    for (int i = 0; i < height_s; i++) {
      ...
      for (int j = 0; j < width_s; j += width_s - 1) {
        ...
        sampled_img[m_s + j] =
            0.25 * image_[m] +
            0.125 * (image_[mN] + image_[mS] + image_[mW] + image_[mE]) +
...
/**
 * \fn template <class T> template <class U> void
 * image<T>::get_histogram(std::vector<U> &histograma, int channel) \brief Get
 * histogram of a channel of image \author Carlos Falcon
 */
template <class T>
template <class U>
void image<T>::get_histogram(
    std::vector<U>& histograma /** histograma de dimension 256*/,
    int channel /**channel to obtain the histogram*/) {
  if (nChannels_ < channel) return;
  if (channel <= 0) return;
...
#pragma omp parallel for
  ...
  for (int i = size * (channel - 1); i < i_end; i++) histograma[image_[i]]++;
}

/**
 * \fn template <class T> void image<T>::init(const int width,const int height,
 * const int nChannels) \brief initializes the image taking memory \author
 * Carlos Falcon
 */
template <class T>
...
void image<T>::set_value(const int x, T value) {
  if (x <= width_ * height_ * nChannels_) image_[x] = value;
}
...
T image<T>::get_value(const int x) {
  T value = 0;
  if (x <= width_ * height_ * nChannels_) return image_[x];
  return value;
}

//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> image<T>::image(const T *vector,const int width,const
 * int height,const int channels) \brief image from T vector \author Carlos
 * Falcon
 */
...
image<T>::image(const T* vector, const int width, const int height,
  ...
  image_.resize(size);
  size = width_ * height_;
#pragma omp parallel shared(size, vector)
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> template<class U> void image<T>::hsv_to_rgb(image<U>
 * &image) \brief function to get colour model RGB from HSV (unsigned char
 * version) \author Carlos Falcon
 */
template <class T>
template <class U>
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> void image<T>::rgb_to_hsv(image<unsigned char>
 * &H,image<unsigned char> &S, image<unsigned char> &V) \brief function to get
 * colour model HSV from RGB (unsigned char version) \author Carlos Falcon
 */
template <class T>
void image<T>::rgb_to_hsv(image<unsigned char>& H, image<unsigned char>& S,
  ...
  for (int i = 0; i < size_; i++) {
    ...
    // H
    if (rgb_max == red) {
      H[i] = (unsigned char)((int)0 +
                             43 * ((int)green - blue) / (rgb_max - rgb_min));
    } else if (rgb_max == green) {
      H[i] = (unsigned char)((int)85 +
                             43 * ((int)blue - red) / (rgb_max - rgb_min));
    } else {
      H[i] = (unsigned char)((int)171 +
                             43 * ((int)red - green) / (rgb_max - rgb_min));
    }
  }
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> void image<T>::set_roi(const int x0,const int x1,const
 * int y0,const int y1) \brief function to set a subwindow border \author Luis
 * Alvarez
 */
template <class T>
...
void image<T>::set_roi(const int x0, const int x1, const int y0, const int y1,
                       const int c0, const int c1) {
  if (x0 > x1 || y0 > y1 || x0 > width_ || y0 > height_ || c0 > nChannels_ ||
      c0 > c1)
    return;
 ...
 */
template <class T>
void image<T>::set_roi(const std::vector<int> roi) {
  roi_ = roi;
}

/////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> template <class U> void  image<T>::set_roi_image(const
 * image<U> &image2) \brief function to fill the image with a subwindow image
 * \author Luis Alvarez
 */

template <class T>
template <class U>
void image<T>::set_roi_image(const image<U>& image2) {
...
#pragma omp parallel shared(x0, x1, y0, y1, c0, c1, width, size, \
                            size_) private(n, i, j, n2, n2_, i2, i2_)
    for (n = c0; n < c1; n++) {
      n2 = (n - c0) * size;
      n2_ = n * size_;
#pragma omp for nowait
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> image<T>  const image<T>::get_roi_image(const
 * std::vector <int> &roi2) const \brief function to get a subwindow image
 * \author Luis Alvarez
 */
template <class T>
...
#pragma omp parallel shared(image2, x0, x1, y0, y1, c0, c1, width, size, \
                            size_) private(n, i, j, n2, n2_, i2, i2_)
  for (n = c0; n < c1; n++) {
    n2 = (n - c0) * size;
    n2_ = n * size_;
...
/**
 * \fn template <class T> template <class U> void
 * image<T>::get_roi_image(image<U> &image2) const \brief function to fill a
 * subwindow image \author Luis Alvarez
 */

template <class T>
template <class U>
void image<T>::get_roi_image(image<U>& image2) const {
  int x0, x1, y0, y1, c0, c1;
...
#pragma omp parallel for shared(k_end) private(k)
  for (k = 0; k < k_end; ++k) image_[k] = image2[k];
 ...
 * image. H goes to the output image to the input image \author Luis Alvarez
 */
template <class T>
...
#pragma omp parallel for shared(H, image2, n, n0, width_1, height_1)
    for (int i = 0; i < height_; i++) {
      for (int j = 0; j < width_; j++) {
        ...
        if (x0i < width_1) {
          if (y0i < height_1) {
            image2[m2 + j] = (T)(dy0_1 * dx0_1 * image_[m0 + x0i] +
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> image<T> const image<T>::resize(const int width, const
 * int height) const \brief function to resize an image \author Luis Alvarez
 */
template <class T>
image<T> const image<T>::resize(
  ...
  if (width_ > width && width_ % width == 0 && height_ > height &&
      height_ % height == 0) {
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> image<T> const image<T>::resize_no_omp(const int
 * width, const int height) const \brief function to resize an image without omp
 * \author Luis Alvarez
 */
template <class T>
image<T> const image<T>::resize_no_omp(const int width,
...
//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn template <class T> T& image<T>::operator()(const int &x,const int
 * &y,const int &channel) \brief operator () to acces image value \author Javier
 * Martin
 */
template <class T>
T& image<T>::operator()(const int& x, const int& y, const int& channel) {
  ...
  if ((x + y * width_ + channel * width_ * height_) >= (image_.size())) {
    ...
    printf("image size()=%d index to accces the vector =%d\n", image_.size(),
           x + y * width_ + channel * width_ * height_);
    int j;
    scanf("%d", &j);
    exit(0);
  }
 ...
 * const T &c) \brief image constructor taking memory and initialiting the
 * channels: R with a value, G with b value and B with c value \author Carlos
 * Falcon
 */
template <class T>
...
#pragma omp parallel for shared(size) private(i)
  for (i = size; i < 2 * size; i++) image_.at(i) = b;
#pragma omp parallel for shared(size) private(i)
  for (i = 2 * size; i < 3 * size; i++) image_.at(i) = c;
}

//////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \fn image<T>::image(const int width,const int height, const int nChannels,
 * const T &a) \brief image constructor taking memory and initialiting with a
 * value \author Luis Alvarez
 */
template <class T>
 ...
 * const int nChannels) \brief image constructor taking memory and initialiting
 * with a value \author Luis Alvarez
 */
template <class T>
image<T>::image(const int width, const int height, const int nChannels) {
  width_ = width;
  ...
  for (int k = 0; k < nChannels_; k++) {        // all of channels
    for (int i = 0; i < height_; i++) {         // reading for columns
      for (int j = 0; j < (width_ / 2); j++) {  // reading for rows
        ...
        image_[(k * height_ * width_) + i * width_ + j] =
            image_[(k * height_ * width_) + i * width_ + width_ - j -
                   1];  // swapping mirrored values in the row
 ...
 * \brief Read an image selecting the library depend on the image format,
 * returns 0 when it can't load the image. \author Pedro Henríquez
 */
template <class T>
