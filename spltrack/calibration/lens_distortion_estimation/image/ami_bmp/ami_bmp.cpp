...
/* FUNCTION TO READ A 24 bits COLOR BMP FORMAT IMAGE  (RETURN 0 IF FINISH
 * PROPERLY -1 OTHERWISE) */
int ami_read_bmp(char name[200], unsigned char** red, unsigned char** green,
  ...
  if (header.type != (int)'M' * 256 + 'B') {
    fprintf(
        stderr,
        "Sorry, the file is not a bmp image file (magic number not correct)\n");
    return (-1);
  }
  ...
  if (infoheader.bits != 24) {
    fprintf(stderr,
            "Sorry, function only implemented for 24 bits colour format\n");
    return (-1);
  }
  /* WE CHECK COMPRESSION */
  if (infoheader.compression != 0) {
    fprintf(stderr,
            "Sorry, function only implemented image without compression \n");
    return (-1);
  }
  /* WE CHECK THE 24 bits */
  ...
  *height = infoheader.height;
  // size = ((*width)%4) ? ((*width)+4-((*width)%4))*(*height) :
  // (*width)*(*height);
  ...
  *blue = (unsigned char*)malloc(size * sizeof(unsigned char));
  ...
  for (i = 0; i < (*height); i++) {
    for (j = 0; j < (*width); j++) {
      (*blue)[i * (*width) + j] = image[cont++];
      (*green)[i * (*width) + j] = image[cont++];
      (*red)[i * (*width) + j] = image[cont++];
    }
    // Ignorar bytes rellenados con 0 para tamaño de fila múltiplo de 4
    // (considerando los 3 bytes RGB)
    cont += (*width) % 4;
  }
...
/* FUNCTION TO WRITE A 24 bits COLOR BMP FORMAT IMAGE  (RETURN 0 IF FINISH
 * PROPERLY -1 OTHERWISE) */
int ami_write_bmp(char name[200], unsigned char* red, unsigned char* green,
  ...
  infoheader.imagesize =
      (3 * width + width % 4) *
      height;  //(width%4) ? 3*(width+4-(width%4))*height : 3*width*height;
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      image[cont++] = blue[i * width + j];
      image[cont++] = green[i * width + j];
      image[cont++] = red[i * width + j];
    }
    // Rellenar bytes con 0 para obtener tamaño de fila múltiplo de 4
    // (considerando los 3 bytes RGB)
